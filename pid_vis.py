...
with Path('pid_test.csv').open('r') as f:
    ...
    header = "LeftSetpoint,LeftVelocity,LeftError,RightSetpoint,RightVelocity,RightError,Heading"
    keys = header.split(",")
    for row in reader:
        for k in keys:
...
ax_vel.set_title("Velocity Values")
ax_vel.plot(lists["LeftSetpoint"], "-b", lists["LeftVelocity"], "-r", lists["LeftError"], "-g",
            lists["RightSetpoint"], "--c", lists["RightVelocity"], "--m", lists["RightError"], "--y")
ax_vel.legend(["Left Setpoint", "Left Velocity", "Left Error", "Right Setpoint", "Right Velocity", "Right Error"])
